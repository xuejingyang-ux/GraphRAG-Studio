...
上传文档时必须选择所属知识库，索引生成的文档、节点和关系都会保存 `kb_id`。问答页面可以选择“自动选择（Supervisor）”或手动指定智能体；自动模式根据命中实体的知识库、实时关键词和通用回退进行路由。回答会显示 `agent_name`、`kb_name` 和 `route_reason`。知识图谱的公共结构为“全局总览 → 知识库 → 类型 → 实体”，因此知识库之间检索隔离，同时全图仍保持连通。
