...
> v1.3.0 第二阶段补充：新增知识库管理和智能体管理页面；智能体配置包含系统提示词、工具、知识库绑定和联网权限；知识库支持独立图谱；自动化验收增加四类路由、手动路由及节点、关系、邻居、导出的知识库隔离测试。

> v1.4.0 第三阶段补充：支持跨知识库多智能体协同和自动路由计划；增加按 `conversation_id + agent_id` 持久化的对话记忆；按查询日志和用户反馈统计调用次数、准确率及平均延迟。
